...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (sum: number, q: Question): number =>
            q.published ? sum + q.points : sum,
        0,
    );
}
...
export function sameType(questions: Question[]): boolean {
    ...
    return questions.every((q: Question): boolean => q.type === firstType);
}
    ...
    return questions.map((q: Question): Question => {
        if (q.id === targetId) {
            ...
            if (targetOptionIndex === -1) {
                updatedOptions = [...q.options, newOption];
            } else {
                updatedOptions = [...q.options];
