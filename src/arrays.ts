...
export function makeMath(addends: number[]): string {
    ...
    const sum = addends.reduce((acc: number, n: number): number => acc + n, 0);
...
export function injectPositive(values: number[]): number[] {
    const negativeIndex = values.findIndex((val: number): boolean => val < 0);
    ...
    if (negativeIndex === -1) {
        ...
        const previousValues = values.slice(0, negativeIndex);
        const sum = previousValues.reduce(
            (total: number, num: number): number => total + num,
            0,
        );
        const result = [...values];
